...
def select_conv(state: State, var_name: str, value) -> None:
    ...
    if state.current_conv is not None:
        state.all_conversations_tables[str(state.current_conv)] = state.conversation_table._dict.copy()
        state.all_conversations[str(state.current_conv)] = state.conversation.copy()
    state.current_conv = value[0][0]

    # Get the new conv table.
    if str(value[0][0]) in state.all_conversations_tables:  # retrieve old conv.
        state.conversation_table = state.all_conversations_tables[str(value[0][0])]._dict.copy()
        state.conversation = state.all_conversations[str(state.current_conv)].copy()
    else:  # start new conv.
        state.conversation_table = {"Conversation": []}
        state.conversation = []
